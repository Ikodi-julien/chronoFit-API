...
* Faire pied de page accueil avec liens utils (site, contact, github, linkedin) => ok
* * Fix bug numérotation rounds => ok
* * Fix bug shrunk rounds => ok (c'était pas un bug, réglage amélioré)
* Faire une video de démo pour youtube => ok
* Faire vue avec video de demo ou direct sur l'accueil ? => ok direct sur l'accueil
* Faire une prèsentation pour linkedin => ok
* Améliorer, faire apparaitre un Exoform à l'ajout d'un round => ok
* 
* Tests de l'app méthodique :
* * Nettoyer les console.log => ok
* * Test scénario Girls:
*   * Accès à la vue des Girls => ok
*   * Les Girls sont affichées => ok
*   * Pas d'erreur ou lien mort => ok
*   * Le choix d'un WOD affiche ReadTrainingView => ok
*   * ReadTrainingView:
*     * Les contrôles ont l'effet attendu => ok
*     * Les comptes de série et de rounds sont corrects => ok
*     * Les éventuels overflow sont pris en compte => ok
*     * Les chronos fonctionnent correctement => ok
*     * Les countdowns fonctionnent correctement => ok
*     * Les bips et la voix fonctionnent de la manière attendue => ok
*     * La modale de validation est déclenchée par:
*       * La validation du dernier exo de la timeline => ok
*       * La fin d'un countdownen dernier exo de la timeline => ok
*       * La fin du time cap => ok
* * Test scénario Custom Training:
*   * Accès à la vue de création d'un training => ok
*   * On peut écrire le nom du training => ok
*   * On peut ajouter un exercice et renseigner ses caractéristiques => ok
*   * On peut ajouter des rounds, déplacer par D&D les rounds et les exos => ok
*   * La validation par "Save" enregistre le training dans le navigateur => ok
*   * La validation par "Delete" supprime le training dans le navigateur => ok
*   * La validation par le chrono affiche ReadTrainingView => ok
*   * ReadTrainingView:
*     * Les contrôles ont l'effet attendu =>
*     * Les comptes de série et de rounds sont corrects =>
*     * Les éventuels overflow sont pris en compte =>
*     * Les chronos fonctionnent correctement =>
*     * Les countdowns fonctionnent correctement =>
*     * Les bips et la voix fonctionnent de la manière attendue =>
*     * La modale de validation est déclenchée par:
*       * La validation du dernier exo de la timeline =>
*       * La fin d'un countdownen dernier exo de la timeline =>
*       * La fin du time cap =>
* * Test de différents navigateurs:
*   * chrome => ok
*   * Firefox:
*     * Fait apparaitre un lag pénible sur la rotation des chronos => ok (Fait keyframes à la place du calcul d'un angle)
*     * Sinon c bon !
*   * Opera => erreur avec SpeechSynthesis => desactivé pour Opera (OPR), sinon c bon
*   * Safari => ?

### Le 21/08 (8h15 - 11h30)

* Faire 5 girls de plus => ok
* Gérer les erreurs:
* * app react => ok (fait composant AppComment)
* * app express => ok (blocs try catch)
* * Les erreurs prennent en compte les loaders => ok
* Faire dernières girls => ok
